...
for(iscena in 1:2){
  ...
  todo2_a[[iscena]]$futile <- array(0, c(2, dim(peff.m)[2], ntriala))
  todo2_a[[iscena]]$present <- matrix(0, nrow = ntriala, ncol = 3)
  todo2_a[[iscena]]$gpower <- rep(0, ntriala)
  
  for(iia in 1:ntriala){
    todo2_a[[iscena]]$pts[,,iia] <- tempa[[iia]]$pts
    todo2_a[[iscena]]$parameter[iia,] <- tempa[[iia]]$parameter
    todo2_a[[iscena]]$futile[,,iia] <- tempa[[iia]]$futile
    todo2_a[[iscena]]$present[iia,] <- tempa[[iia]]$present
...
save.image("TODO2_parameter_optimization.RData")
